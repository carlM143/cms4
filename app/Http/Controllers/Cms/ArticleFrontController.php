...
class ArticleFrontController extends Controller
{
    ...
    public function categories($criteria){
        ...
        $categories = DB::select('SELECT ifnull(c.name, "Uncategorized") as cat, ifnull(c.id,0) as cid,count(ifnull(c.id,0)) as total_articles FROM `articles` a left join article_categories c on c.id=a.category_id where a.deleted_at is null and c.deleted_at is null and status="Published" GROUP BY c.name,c.id ORDER BY c.name');
